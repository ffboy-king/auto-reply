...
DB_PATH = os.path.expanduser("~/xirus_master_pro.db")
...
# Unique Primary Key on 'phone' allows 1 user to add 100+ numbers.
def init_db():
...
active_sessions = {} # Dictionary to store all live clients

async def start_auto_reply_engine(phone, session_str, reply_text):
    """Handles individual account monitoring in the background."""
...
async def master_router(event):
    ...
    if text == "/start" or text == "🏠 Home":
        ...
        for p in list(active_sessions.keys()):
            # This logic should ideally check owner_id, for simplicity here:
            await active_sessions[p].disconnect()
    ...
    # --- ADMIN PRIVILEGES ---
        ...
        await event.respond(details[:4000])
...
async def run_account_setup(uid):
    try:
        async with bot.conversation(uid, timeout=600) as conv:
            ...
            await bot.send_message(uid, f"**🚀 {style('Account Setup Done!')}**\n\n{style('Aap 100+ numbers aise hi add kar sakte hain.')}", buttons=main_kb())
...
if __name__ == '__main__':
    print("✅ XIRUS MODZ 🥲!")
    asyncio.get_event_loop().create_task(recovery_on_boot())
